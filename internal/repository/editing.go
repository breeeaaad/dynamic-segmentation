...
func Addsegments(conn *pgx.Conn, add helpers.Add) error {
	...
	if _, err := conn.Exec(
		context.Background(),
		"insert into history(user_id,added_at,segment_id) select $1,$2,segment_id from segments where segment_name=any($3)",
		add.Id, time.Now(), add.Addsegments,
	); err != nil {
		return err
	}
...
func Delsegments(conn *pgx.Conn, add helpers.Add) error {
	if _, err := conn.Exec(context.Background(),
		"update history set deleted_at=$1 where user_id=$2 and segment_id in (select segment_id from segments where segment_name=any($3))",
